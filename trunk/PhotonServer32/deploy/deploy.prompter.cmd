...
echo            1.  Lite
echo            2.  Lite Lobby
echo            3.  Policy (Unity, Flash, Silverlight)
echo            4.  Counter Publisher
echo            5.  Mmo Demo
echo            6.  Loadbalancing
echo.
echo            9.  All
echo.
echo            0.  Exit
...
if '%choice%'=='2' call :litelobby
if '%choice%'=='3' call :policy
if '%choice%'=='4' call :counterpublisher
if '%choice%'=='5' call :mmo
if '%choice%'=='6' call :loadbalancing
...
:litelobby
set rootpath=LiteLobby
set slnfile=LiteLobby.sln
set binpath=LiteLobby\\
set dst=LiteLobby
%WINDIR%\Microsoft.NET\Framework\v4.0.30319\MSBuild.exe %buildfile% /verbosity:%verbosity% /l:FileLogger,Microsoft.Build.Engine;logfile=log\%rootpath%Build.log;verbosity=%verbosity%;performancesummary /property:slnfile="%slnfile%" /property:binp="%binpath%" /property:rootpath="%rootpath%" /property:dst="%dst%" /t:BuildAndCopyForDeploy
goto done
...
:policy
set rootpath=Policy
set slnfile=Policy.2008.sln
set binpath=Policy.Application\\
set dst=Policy
%WINDIR%\Microsoft.NET\Framework\v4.0.30319\MSBuild.exe %buildfile% /verbosity:%verbosity% /l:FileLogger,Microsoft.Build.Engine;logfile=log\%rootpath%Build.log;verbosity=%verbosity%;performancesummary /property:slnfile="%slnfile%" /property:binp="%binpath%" /property:rootpath="%rootpath%" /property:dst="%dst%" /t:BuildAndCopyForDeploy
goto done
