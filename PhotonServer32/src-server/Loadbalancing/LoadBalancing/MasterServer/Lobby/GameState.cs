...
namespace Photon.LoadBalancing.MasterServer.Lobby
{
    ...
    public class GameState
    {
        ...
        private readonly Dictionary<string, PeerState> peerDict = new Dictionary<string, PeerState>();
        ...
        public void CheckJoinTimeOuts(DateTime minDateTime)
        {
            ...
            while (node != null)
            {
                ...
                if (peerState.UtcCreated < minDateTime)
                {
                    ...
                    if (string.IsNullOrEmpty(peerState.UserId) == false)
                    {
                        this.peerDict.Remove(peerState.UserId);
                    }
                }
        ...
        public bool TryGetPeerState(string userId, out PeerState peerState)
        {
            return this.peerDict.TryGetValue(userId, out peerState);
        }
        ...
        public override string ToString()
        {
            return
                string.Format(
                    "GameState {0}: PlayerCount: {1}, Created on GS: {2} at {3}, GSPlayerCount: {4}, IsOpen: {5}, IsVisibleInLobby: {6}, IsVisible: {7}, UtcCreated: {8}",
                    this.Id,
                    this.PlayerCount,
        ...
        private void OnPeerJoinedGameServer(string userId)
        {
            ...
            if (this.peerDict.ContainsKey(userId))
            {
                return;
            }

            var peerState = new PeerState(userId);
            this.peerDict.Add(peerState.UserId, peerState);

            if (log.IsWarnEnabled)
            {
                log.WarnFormat("Unknown user joined:  gameId={0}, userId={1}", this.Id, userId);
            }
        }
